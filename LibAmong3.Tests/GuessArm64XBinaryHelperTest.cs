...
namespace LibAmong3.Tests
{
    public class GuessArm64XBinaryHelperTest
    {
        ...
        public void GuessByCHPE(string dllName, Arm64XBinaryForm form)
        {
            var bytes = File.ReadAllBytes($@"Files\{dllName}");
            var guessr = new GuessArm64XBinaryHelper();
            Assert.That(guessr.Guess(bytes, new Guess1Options { LookAtCHPEMetadataVersion = true, }), Is.EqualTo(form));
        }
        ...
        [TestCase("dllmain-x86.obj", Arm64XBinaryForm.X86Coff)]
        [TestCase("dllmain-arm64x.obj", Arm64XBinaryForm.Arm64XCoffUponX86Coff)]
        public void GuessByCHPEAndDvrt(string dllName, Arm64XBinaryForm form)
        {
            var bytes = File.ReadAllBytes($@"Files\{dllName}");
            var guessr = new GuessArm64XBinaryHelper();
            Assert.That(guessr.Guess(bytes, new Guess1Options { LookAtCHPEMetadataVersion = true, LookAtDvrtHeader = true, }), Is.EqualTo(form));
        }
        ...
        [TestCase("dllmain-x86.obj", Arm64XBinaryForm.X86Coff)]
        [TestCase("dllmain-arm64x.obj", Arm64XBinaryForm.Arm64XCoffUponX86Coff)]
        public void GuessByDvrt(string dllName, Arm64XBinaryForm form)
        {
            var bytes = File.ReadAllBytes($@"Files\{dllName}");
            var guessr = new GuessArm64XBinaryHelper();
            Assert.That(guessr.Guess(bytes, new Guess1Options { LookAtDvrtHeader = true, }), Is.EqualTo(form));
        }
